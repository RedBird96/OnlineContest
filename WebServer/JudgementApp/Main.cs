...
namespace JudgementApp
{
    public class Main
    {
        ...
        public static bool CheckUser(string name,string ProblemName,long FKCompany, string UserEmail)
        {
            ...
            check = SQL.ScalarQuery("SELECT CASE WHEN EXISTS (SELECT TOP 1 * FROM Judgement  WHERE Name = '" + name + "' and UserEmail='"+ UserEmail + "' and ProblemName='" + ProblemName + "' and FKCompany=" + FKCompany+" and date = (select CONVERT(datetime, '" + dateTime.ToString("yyyy/MM/dd") + "', 20))) THEN CAST (1 AS BIT) ELSE CAST (0 AS BIT) END");
